...
test('Fleet snapshot and due-list features are owned by their canonical functions', () => {
  ...
  assert.match(maintenance, /fleetListMode==='due30'\?'<span>Booking type<\/span><span aria-label="Already booked"><\/span><span>Booked<\/span><span>Email sent<\/span>'/, 'the due-30 table must retain the requested right-side headings');
  assert.match(maintenance, /fleetShortVehicle\(v\.model\)/, 'the due-30 vehicle column must show only the first model word');
  assert.match(maintenance, /fleetWorkDueGroupCell\(group\.items\)\+fleetBookingTypeGroupCell\(group\.items\)\+fleetBookingTickGroupCell\(group\.items\)\+fleetBookedGroupCell\(group\.items\)\+fleetEmailSentCell\(group\)/, 'work due, booking type, booked tick, booked date and email audit must retain dedicated ordered columns');
