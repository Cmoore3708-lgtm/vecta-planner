...
test('phone Fleet due list uses a screen-width compact grid',()=>{
  ...
  assert.match(html,/\.fleetRow\.due30Columns\{box-sizing:border-box;grid-template-columns:minmax\(104px,118px\) minmax\(54px,1fr\) 18px 96px!important;grid-template-rows:28px auto auto;[\s\S]*?width:calc\(100% - 8px\);max-width:calc\(100% - 8px\)/);
  assert.match(html,/\.fleetRow\.due30Columns>:nth-child\(2\)\{grid-column:1;grid-row:2;[\s\S]*?padding-left:2px\}/);
  assert.match(html,/\.fleetRow\.due30Columns>:nth-child\(3\)\{grid-column:2\/4;grid-row:1\/3;[\s\S]*?align-self:center/);
...
test('Fleet due list links booked dates to planner days and dates email audit ticks',()=>{
  ...
  assert.match(html,/grid-template-columns:125px 76px minmax\(105px,1fr\) 88px 56px 20px 58px 54px!important/, 'Work due, booking type, booked tick, booked date and Email sent must have compact dedicated columns');
  ...
  assert.match(html,/<span>Booking type<\/span><span aria-label="Already booked"><\/span><span>Booked<\/span><span>Email sent<\/span>/, 'the due list headers must use the requested order');
  assert.match(html,/fleetWorkDueGroupCell\(group\.items\)\+fleetBookingTypeGroupCell\(group\.items\)\+fleetBookingTickGroupCell\(group\.items\)\+fleetBookedGroupCell\(group\.items\)\+fleetEmailSentCell\(group\)/, 'Email sent must remain the far-right cell');
  assert.match(html,/function fleetBookingTypeGroupCell\(items\)[\s\S]*?fleetBookingTypeEntry[\s\S]*?fleetDueIcon\(item\.plan&&item\.plan\.type\|\|''\)[\s\S]*?function fleetBookingTickGroupCell\(items\)[\s\S]*?fleetBookingTickEntry[\s\S]*?fleetBookedTick\(item\.bookedJob\)/, 'the red booked tick must have its own fixed column without shifting booking type');
