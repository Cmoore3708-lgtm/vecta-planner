...
export default async function handler(req,res){
 ...
 try{
  ...
  if(!confirmed){await rest(url,key,'website_booking_requests','POST',request);await sendBookingBadges();return res.status(201).json({ok:true,confirmed:false,request_id:id});}
  ...
  const vehicleRows=await rest(url,key,`vehicles?select=id,registration&registration=eq.${encodeURIComponent(job.registration)}&limit=1`);
  let vehicleId=vehicleRows?.[0]?.id||null;
  if(vehicleRows?.length)await rest(url,key,`vehicles?registration=eq.${encodeURIComponent(job.registration)}`,'PATCH',{customer_id:customerId,vehicle:job.vehicle});else{const created=await rest(url,key,'vehicles','POST',{registration:job.registration,customer_id:customerId,vehicle:job.vehicle,notes:'Website booking'});vehicleId=created?.[0]?.id||null;}
  await rest(url,key,`jobs?id=eq.${jobId}`,'PATCH',{customer_id:customerId,vehicle_id:vehicleId});
