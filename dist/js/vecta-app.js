...
var VECTA_APP_VERSION='v348-test-manual-service-mileage';
...
function registerVectaServiceWorker(){
  ...
  window.addEventListener('load',function(){
    Promise.resolve(window.__vectaCacheResetPromise).finally(function(){
      navigator.serviceWorker.register('/service-worker.js?v=20260914-test-manual-service-mileage-v348',{updateViaCache:'none'}).then(function(reg){
...
function invoiceArchiveMobileNumber(inv){var raw=String(inv&&inv.invoice_number||'Draft'),match=raw.match(/(\d+)$/);return match?'#'+match[1]:raw}
function invoiceArchiveMobileDate(inv){var raw=String(inv&&inv.invoice_date||'').slice(0,10),parts=raw.split('-');return parts.length===3?parts[2]+'/'+parts[1]+'/'+parts[0].slice(2):'—'}
function invoiceArchiveRows(query){
  ...
  return '<div class="invoiceArchiveTable"><div class="invoiceArchiveHead"><span>Invoice</span><span>Registration / Account</span><span>Customer</span><span>Date</span><span>Invoice Total</span><span>Payment Method / Action</span></div>'+
    list.map(function(inv){var overdue=isInvoicePaymentMethodOverdue(inv),contractor=isContractorInvoice(inv);return '<div class="invoiceArchiveRow '+(contractor?'contractorInvoiceRow ':'')+(overdue?'paymentMethodOverdue':'')+'" data-open-invoice="'+inv.id+'"><span class="invoiceArchiveNumber"><span class="invoiceArchiveNumberFull">'+esc(inv.invoice_number||'Draft')+'</span><span class="invoiceArchiveNumberMobile">'+esc(invoiceArchiveMobileNumber(inv))+'</span>'+(contractor?'<em class="contractorTermsBadge">CONTRACTOR · 30 DAY TERMS</em>':'')+(overdue?'<em class="paymentChaseBadge">CHASE PAYMENT METHOD</em>':'')+'</span><span class="invoiceArchiveReference"><span class="invoiceArchiveReferenceFull">'+invoiceArchiveReferenceHtml(inv)+'</span><span class="invoiceArchiveReferenceMobile">'+esc(inv.registration||'ACCOUNT')+'</span></span><span class="invoiceArchiveCustomer">'+esc(inv.customer_name||'—')+'</span><span class="invoiceArchiveDate"><span class="invoiceArchiveDateFull">'+esc(inv.invoice_date?niceDate(String(inv.invoice_date).slice(0,10)):'—')+'</span><span class="invoiceArchiveDateMobile">'+esc(invoiceArchiveMobileDate(inv))+'</span></span><strong class="invoiceArchiveTotal">'+money(inv.total||0)+'</strong><span class="invoicePaymentQuick"><select class="'+invoicePaymentMethodClass(inv.payment_method)+'" data-payment-method="'+esc(inv.id)+'" aria-label="Payment method for '+esc(inv.invoice_number||'invoice')+'">'+invoicePaymentMethodOptions(inv.payment_method)+'</select><button type="button" class="btn danger invoiceDeleteBtn" data-delete-invoice="'+esc(inv.id)+'" aria-label="Cancel '+esc(inv.invoice_number||'invoice')+'">Cancel</button></span></div>'}).join('')+
    '</div>';
}
function cancelledInvoiceRowsHtml(){
  ...
  return '<div class="card panel invoiceArchiveResults cancelledInvoiceResults"><h3>Cancelled invoices</h3><p class="muted">Retained for accounting safety and excluded from live totals. Restore is available for 30 days.</p><div class="invoiceArchiveTable">'+list.map(function(inv){var days=Math.max(0,VECTA_INVOICE_RESTORE_DAYS-vectaInvoiceVoidAgeDays(inv));return '<div class="invoiceArchiveRow"><span class="invoiceArchiveNumber"><span class="invoiceArchiveNumberFull">'+esc(inv.invoice_number||'Invoice')+'</span><span class="invoiceArchiveNumberMobile">'+esc(invoiceArchiveMobileNumber(inv))+'</span><em class="paymentChaseBadge">CANCELLED</em></span><span class="invoiceArchiveReference"><span class="invoiceArchiveReferenceFull">'+invoiceArchiveReferenceHtml(inv)+'</span><span class="invoiceArchiveReferenceMobile">'+esc(inv.registration||'ACCOUNT')+'</span></span><span class="invoiceArchiveCustomer">'+esc(inv.customer_name||'—')+'</span><span class="invoiceArchiveDate"><span class="invoiceArchiveDateFull">'+esc(inv.invoice_date?niceDate(String(inv.invoice_date).slice(0,10)):'—')+'</span><span class="invoiceArchiveDateMobile">'+esc(invoiceArchiveMobileDate(inv))+'</span></span><strong class="invoiceArchiveTotal">'+money(inv.total||0)+'</strong><span class="invoiceRestoreQuick">'+(vectaInvoiceCanRestore(inv)?'<button type="button" class="btn" data-restore-invoice="'+esc(inv.id)+'" aria-label="Restore '+esc(inv.invoice_number||'invoice')+'">Restore · '+days+' days left</button>':'Retained record')+'</span></div>'}).join('')+'</div></div>'
}
...
function normaliseServiceMileage(value){
  var mileage=String(value===undefined||value===null?'':value).trim().replace(/\s*miles?\s*$/i,'').replace(/[\s,]/g,'');
  return /^\d{1,7}$/.test(mileage)?mileage:'';
}
